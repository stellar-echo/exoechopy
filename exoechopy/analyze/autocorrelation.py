...
__all__ = ['autocorrelate_array', 'autocorrelate_array_w_nans',
           'autocorrelation_overlapping_windows', 'period_folded_autocorrelation',
...
def autocorrelate_array_w_nans(data_array: (u.Quantity, np.ndarray),
    ...
    results = np.zeros(max_lag+1)
    for lag in range(max_lag+1):
        unsum = data_array[lag:] * data_array[:len(data_array)-lag]
        num_unsum = np.sum(~np.isnan(unsum))
        results[lag] = np.nansum(unsum)/num_unsum
