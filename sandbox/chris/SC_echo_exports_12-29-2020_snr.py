...
import_file_folder = 'SC_disk_echo_search_v3_snr_weight'
export_file_folder = 'SC_disk_echo_export_deltas_only_snr'
...
# Number of targets to interrogate, None for all:
num_targets = None
...
# Decay threshold--if the intensity is higher than these values 1 or 2 indices after the flare, reject as outlier:
...
# Where we load results from:
import_fp = cwd.parent.parent / "disks" / import_file_folder
...
all_files = os.listdir(import_fp)
...
with open(import_fp / clean_delta_flare_stars_filename, 'r') as tnames:
