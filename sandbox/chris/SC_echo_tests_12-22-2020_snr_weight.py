...
file_folder = 'SC_disk_echo_search_v3_snr_weight'

# List of all targets to interrogate:
all_stars_filename = 'original_list_stars_with_flares.txt'
...
conf_range = 99.7
# At how many sigma is an echo detected?
stat_meaningful_sigma = 3
...
def echo_analysis_do_it_all(all_flare_arr: np.ndarray, flare_peak_index=0):
    ...
    Weight is calculated by (np.nanmax(flare) - 1)/stdDev(post-flare-LC)
    ...
    for study_i, flare_indices in enumerate(flare_ind_lists):
        ...
        statistically_interesting_indices = \
            np.nonzero((boot_conf[0][echo_slice] > 0)
                       & ((final_normed_mean - stat_meaningful_sigma * final_normed_std_dev)[
                              echo_slice] > 0))[0]
        ...
        if generate_figures and (study_i < num_reinjection_verbosity or len(statistically_interesting_indices) > 0):
            ...
            eep.visualize.plot_signal_w_uncertainty(
                ...
                (final_normed_mean + stat_meaningful_sigma * final_normed_std_dev)[echo_slice],
                (final_normed_mean - stat_meaningful_sigma * final_normed_std_dev)[echo_slice],
                ...
                uncertainty_label="±" + str(stat_meaningful_sigma) + " sigma of mean",
                uncertainty_color='salmon',
                save='hold',
                axes_object=ax)
