...
filename = "kepler_data_with_echoes_Nano.npy"
filepath = Path(fp)
...
flare_indices = eep.analyze.find_peaks_stddev_thresh(mean_subtracted_data,
...
ax[1].plot(mean_subtracted_data)
ax[1].scatter(flare_indices, mean_subtracted_data[flare_indices], color='g', label='flare local')
...
eep.visualize.plot_flare_array(mean_subtracted_data, flare_indices,
                               back_pad=pre_flare_pad, forward_pad=max_lag_inds,
                               display_index=True)

full_autocorr = eep.analyze.autocorrelate_array(mean_subtracted_data, max_lag=max_lag_inds)
...
for flare_index in flare_indices:
    ...
    if flare_index - pre_flare_pad > 0 and flare_index + post_flare_pad < len(mean_subtracted_data):
        flare_curve = mean_subtracted_data[flare_index - pre_flare_pad:flare_index + post_flare_pad]
        ...
        ax[0].scatter([flare_index], mean_subtracted_data[flare_index], color='r')
