...
def view_flares(path):
    ...
    ax[1].axhline(3*np.std(lc_detr.flux) + np.median(lc_detr.flux), c="k", alpha=0.5, linestyle="dotted", label="3 sigma threshold")
    ax[1].axhline(6*np.std(lc_detr.flux) + np.median(lc_detr.flux), c="k", alpha=0.5, linestyle="dashed", label="6 sigma threshold")
    #plt.xlabel("Index")
